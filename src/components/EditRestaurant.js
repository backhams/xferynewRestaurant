...
export default function EditRestaurant() {
        ...
        React.useCallback(() => {
            const fetchData = async () => {
                try {
                    ...
                    if (userEmail) {
                        const response = await fetch(`http://192.168.20.86:5000/restaurantProfileData?email=${userEmail}`, {
                            method: 'GET',
                            headers: {
    ...
    const handleUpdatePress = async () => {
        try {
            ...
            const response = await fetch('http://192.168.20.86:5000/restaurantProfileEdit', {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                },
