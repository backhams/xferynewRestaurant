...
const MenuPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.72.86:5000/nearbySearch?page=${page}&latitude=${latitude}&longitude=${longitude}`, {
        method: 'GET',
        headers: {
  ...
  const handleLoadMore = async () => {
    if (!loading && !fetchingMore && hasMore) {
      ...
      try {
        const response = await fetch(`http://192.168.72.86:5000/nearbySearch?page=${page + 1}&latitude=${latitude}&longitude=${longitude}`, { // Use page + 1 directly
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
